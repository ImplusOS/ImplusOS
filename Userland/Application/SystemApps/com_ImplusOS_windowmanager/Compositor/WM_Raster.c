...
static uint32_t clamp_index_i32(int32_t value, uint32_t limit)
{
...
{
    if (!canvas || !canvas->pixels || radius == 0u) return;
    rect = wm_rect_intersection(rect, canvas->clip);
    if (rect.w == 0u || rect.h == 0u) return;
    if (radius > 48u) radius = 48u;
    uint64_t pixel_count64 = (uint64_t)rect.w * (uint64_t)rect.h;
    if (pixel_count64 == 0u || pixel_count64 > SIZE_MAX / sizeof(uint32_t)) return;
    if (pixel_count64 > 2200000u && radius > 18u) radius = 18u;
    uint32_t *temporary = (uint32_t *)malloc((size_t)pixel_count64 * sizeof(uint32_t));
    if (!temporary) return;

    uint32_t kernel = radius * 2u + 1u;
    for (uint32_t row = 0u; row < rect.h; ++row) {
        uint32_t source_y = (uint32_t)rect.y + row;
        uint32_t red = 0u, green = 0u, blue = 0u;
        for (int32_t sample = -(int32_t)radius; sample <= (int32_t)radius; ++sample) {
            uint32_t sx = clamp_index_i32(sample, rect.w);
            uint32_t color = canvas->pixels[source_y * canvas->stride +
                (uint32_t)rect.x + sx];
            red += (color >> 16u) & 0xFFu;
            green += (color >> 8u) & 0xFFu;
            blue += color & 0xFFu;
        }
        for (uint32_t col = 0u; col < rect.w; ++col) {
            temporary[row * rect.w + col] = 0xFF000000u |
                ((red / kernel) << 16u) | ((green / kernel) << 8u) |
                (blue / kernel);
            uint32_t remove_x = clamp_index_i32((int32_t)col - (int32_t)radius,
                                                rect.w);
            uint32_t add_x = clamp_index_i32((int32_t)col + (int32_t)radius + 1,
                                             rect.w);
            uint32_t remove_color = canvas->pixels[source_y * canvas->stride +
                (uint32_t)rect.x + remove_x];
            uint32_t add_color = canvas->pixels[source_y * canvas->stride +
                (uint32_t)rect.x + add_x];
            red = red - ((remove_color >> 16u) & 0xFFu) +
                  ((add_color >> 16u) & 0xFFu);
            green = green - ((remove_color >> 8u) & 0xFFu) +
                    ((add_color >> 8u) & 0xFFu);
            blue = blue - (remove_color & 0xFFu) + (add_color & 0xFFu);
        }
    }
    ...
    for (uint32_t col = 0u; col < rect.w; ++col) {
        uint32_t red = 0u, green = 0u, blue = 0u;
        for (int32_t sample = -(int32_t)radius; sample <= (int32_t)radius; ++sample) {
            uint32_t sy = clamp_index_i32(sample, rect.h);
            uint32_t color = temporary[sy * rect.w + col];
            red += (color >> 16u) & 0xFFu;
            green += (color >> 8u) & 0xFFu;
            blue += color & 0xFFu;
        }
        for (uint32_t row = 0u; row < rect.h; ++row) {
            uint32_t *destination = &canvas->pixels[
                ((uint32_t)rect.y + row) * canvas->stride + (uint32_t)rect.x + col];
            *destination = 0xFF000000u | ((red / kernel) << 16u) |
                ((green / kernel) << 8u) | (blue / kernel);
            uint32_t remove_y = clamp_index_i32((int32_t)row - (int32_t)radius,
                                                rect.h);
            uint32_t add_y = clamp_index_i32((int32_t)row + (int32_t)radius + 1,
                                             rect.h);
            uint32_t remove_color = temporary[remove_y * rect.w + col];
            uint32_t add_color = temporary[add_y * rect.w + col];
            red = red - ((remove_color >> 16u) & 0xFFu) +
                  ((add_color >> 16u) & 0xFFu);
            green = green - ((remove_color >> 8u) & 0xFFu) +
                    ((add_color >> 8u) & 0xFFu);
