...
static wm_glyph_t *glyph_get(wm_font_impl_t *impl, int codepoint, float pixel_height)
{
    ...
    float scale = stbtt_ScaleForPixelHeight(&impl->info, pixel_height);
    int advance = 0;
    int left_bearing = 0;
    int x0 = 0, y0 = 0, x1 = 0, y1 = 0;
    stbtt_GetCodepointHMetrics(&impl->info, codepoint, &advance, &left_bearing);
    stbtt_GetCodepointBitmapBox(&impl->info, codepoint, scale, scale, &x0, &y0, &x1, &y1);
    (void)left_bearing;
    ...
    candidate->advance = (int)((float)advance * scale);
    ...
    if (candidate->width > 0 && candidate->height > 0 &&
        ...
        if (candidate->bitmap) {
            stbtt_MakeCodepointBitmap(&impl->info, candidate->bitmap,
                candidate->width, candidate->height, candidate->width,
...
bool wm_font_init(wm_font_t *font, const char *path)
{
    ...
    if (bytes != (int64_t)stat.size ||
        !stbtt_InitFont(&impl->info, impl->buffer,
                        stbtt_GetFontOffsetForIndex(impl->buffer, 0))) {
        free(impl->buffer);
        free(impl);
        return false;
    }
...
uint32_t wm_font_measure(wm_font_t *font, const char *text, float pixel_height)
{
    ...
    while (*text) {
        ...
        if (previous)
            width += (int)((float)stbtt_GetCodepointKernAdvance(
                &impl->info, previous, codepoint) * scale);
                  ...
                  uint32_t color, float pixel_height, uint32_t max_width)
{
    ...
    (void)descent;
    (void)line_gap;
    int32_t baseline = y + (int32_t)((float)ascent * scale);
    int32_t cursor_x = x;
    int previous = 0;
    ...
    while (*text) {
        int codepoint = utf8_decode(&text);
        if (previous)
            cursor_x += (int32_t)((float)stbtt_GetCodepointKernAdvance(
                &impl->info, previous, codepoint) * scale);
        wm_glyph_t *glyph = glyph_get(impl, codepoint, pixel_height);
        if (!glyph) continue;
        if (max_width != 0u && cursor_x - x >= (int32_t)max_width) break;
        if (glyph->bitmap) {
