...
void wm_ipc_handle_message(wm_state_t *state, const ipc_message_t *message)
{
    ...
    case WM_UPDATE_CLOCK: {
        ...
        if (!message_has(message, sizeof(wm_msg_header_t) + 8u)) break;
        const struct clock_message *command = (const void *)message->data;
        strncpy(state->clock_text, command->time, sizeof(state->clock_text) - 1u);
        if (message_has(message, sizeof(struct clock_message)))
            strncpy(state->date_text, command->date, sizeof(state->date_text) - 1u);
