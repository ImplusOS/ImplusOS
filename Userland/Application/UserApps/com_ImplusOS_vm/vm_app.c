...
#define GUEST_RAM_SIZE  (64ULL * 1024ULL * 1024ULL)
#define FB_W 800
#define FB_H 600
...
static uint32_t handle_io_in(uint16_t port) {
    ...
    case PS2_DATA:        return 0x00;
    case PS2_STATUS:      return 0x00;
...
static void handle_io_out(uint16_t port, uint32_t data) {
    ...
    case PIT_CH0:  case PIT_CMD:
    case PS2_DATA: case PS2_STATUS:
    case PORT_A20:
...
void _start(void)
{
    ...
    if (ovmf_fd >= 0) {
        ...
        void *ovmf_ram = os_mmap(ovmf_map_size, 0);
        if (ovmf_ram) {
            memset(ovmf_ram, 0, ovmf_map_size);

            int64_t total = 0, chunk;
            while ((chunk = file_read(ovmf_fd, (uint8_t *)ovmf_ram + total,
                    (uint32_t)(ovmf_map_size - (uint64_t)total))) > 0) {
                total += chunk;
            }
            if (total > 0) {
                uint64_t offset = ovmf_map_size - (uint64_t)total;
                if (offset > 0) {
                    memmove((uint8_t *)ovmf_ram + offset, ovmf_ram, (size_t)total);
                    memset(ovmf_ram, 0xFF, (size_t)offset);
                }
                ...
                if (kvm_ioctl(kvm_fd, KVM_IOCTL_MAP_MEMORY, (uint64_t)(uintptr_t)&rom_region) == 0) {
                    using_ovmf = 1;
                }
            }
        }
    ...
    uint32_t exit_counts[64];
    memset(exit_counts, 0, sizeof(exit_counts));
    ...
    while (running) {
        ...
        if (ei.exit_reason < 64) exit_counts[ei.exit_reason]++;
        switch (ei.exit_reason) {
        case EXIT_IO:
            hlt_n = 0;
            if (ei.io_direction == 0) {
                if (ei.io_port == COM1_PORT) {
                    char c = (char)(ei.io_data & 0xFF);
                    char b[2] = { c, 0 };
                    vga_putc((uint8_t *)guest_ram, c);
                }
        ...
        case EXIT_EPT_MISCONFIG:
            {
                ...
                if (page) {
                    ...
                    if (kvm_ioctl(kvm_fd, KVM_IOCTL_MAP_MEMORY,
                                  (uint64_t)(uintptr_t)&dummy_region) < 0) {
                        running = 0;
                    }
                } else {
                    running = 0;
        ...
        case EXIT_EXT_INT: case EXIT_EXCEPTION_NMI:
            hlt_n = 0;
            unhandled_exit_count = 0;
            break;
        default:
            if (++unhandled_exit_count > 10000) {
                running = 0;
            }
            break;
        }
