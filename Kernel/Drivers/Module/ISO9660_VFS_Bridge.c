...
/* ---- driver_manager_find() lookup ---- */

static const iso9660_driver_t *iso9660_get_driver(void)
{
...
{
    const iso9660_driver_t *drv = iso9660_get_driver();
    if (!drv || !drv->init) {
        return false;
    }
    return drv->init();
...
/* ---- vfs_file_t <-> ISO9660_FILE conversion (former ISO9660_VFS_Adapter.c) ----
 * ISO9660 is read-only media: write_file/write_at/truncate/creat/mkdir/unlink
 * all report failure, matching the original adapter. */

static bool iso9660_vfs_find_file(const char *path, vfs_file_t *out_file)
{
    const iso9660_driver_t *drv = iso9660_get_driver();
    if (!drv || !drv->find_file) {
        return false;
    }
    ...
    if (!drv->find_file(path, iso_file)) {
        free(iso_file);
        return false;
    }
...
{
    const iso9660_driver_t *drv = iso9660_get_driver();
    if (!file || !file->driver_data || !drv || !drv->read_file) {
        return false;
    }
    return drv->read_file((ISO9660_FILE *)file->driver_data, buffer);
}
...
{
    const iso9660_driver_t *drv = iso9660_get_driver();
    if (!file || !file->driver_data || !drv || !drv->read_at) {
        return false;
    }
    return drv->read_at((ISO9660_FILE *)file->driver_data, offset, buffer, size);
}
...
{
    const iso9660_driver_t *drv = iso9660_get_driver();
    if (!file || !file->driver_data || !drv || !drv->get_file_size) {
        return 0;
    }
    return drv->get_file_size((ISO9660_FILE *)file->driver_data);
}
...
{
    const iso9660_driver_t *drv = iso9660_get_driver();
    if (!out_entry || !drv || !drv->readdir) {
        return -1;
    }

    ISO9660_DIRENT iso_dirent;
    memset(&iso_dirent, 0, sizeof(iso_dirent));
    int32_t result = drv->readdir(handle, &iso_dirent);
    if (result <= 0) {
        return result;
    }
...
static void iso9660_vfs_list_root(void)
{
    const iso9660_driver_t *drv = iso9660_get_driver();
    if (drv && drv->list_root_files) {
        drv->list_root_files();
    }
}
...
static const vfs_driver_t g_iso9660_vfs_driver = {
    ...
    .list_root = iso9660_vfs_list_root,
};
