                                        ...
                                        uint64_t *entry_out)
{
    ...
    if (!elf_loader_load_module_image_from_memory(file_data, file_size, policy, &module)) {
        return false;
    }
    ...
    if (!failed && ehdr->e_shoff != 0 && ehdr->e_shnum != 0) {
        if (ehdr->e_shentsize != sizeof(Elf64_Shdr)) {
            ...
            for (uint16_t sec = 0; sec < ehdr->e_shnum && !failed; ++sec) {
                ...
                if (rel_count > policy->max_relocation_count - relocation_count) {
                    failed = true;
                    break;
                }
                ...
                for (uint32_t r = 0; r < rel_count; ++r) {
                    ...
                    if (where_addr % sizeof(uint64_t) != 0) {
                        failed = true;
                        break;
                    }
                    ...
                    if (type == R_X86_64_RELATIVE) {
                        *where = (uint64_t)((int64_t)load_bias + rela->r_addend);
                        continue;
                    }
                    ...
                    if (type == R_X86_64_64 ||
                        ...
                        *where = (uint64_t)((int64_t)sym_addr + rela->r_addend);
                        continue;
                    }
