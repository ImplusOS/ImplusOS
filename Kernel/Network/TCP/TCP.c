...
{
    g_tcp_seq_counter += 64000u + (uint32_t)(timer_ticks() & 0xFFFFu);
    return g_tcp_seq_counter;
}
...
static void recv_buf_write(tcp_connection_t *c, const uint8_t *data, uint16_t len)
{
    for (uint16_t i = 0; i < len; ++i) {
        if (c->recv_count >= TCP_RECV_BUF_SIZE) break;
        c->recv_buf[c->recv_head] = data[i];
        c->recv_head = (uint16_t)((c->recv_head + 1u) % TCP_RECV_BUF_SIZE);
        c->recv_count++;
    }
}
...
{
    uint16_t read = 0;
    while (read < len && c->recv_count > 0u) {
        buf[read++] = c->recv_buf[c->recv_tail];
        c->recv_tail = (uint16_t)((c->recv_tail + 1u) % TCP_RECV_BUF_SIZE);
        c->recv_count--;
    }
    return read;
}
...
int32_t tcp_send(int32_t conn_id, const void *data, uint16_t len)
{
    ...
    while (sent < len) {
        ...
        for (uint16_t i = 0; i < chunk; ++i) {
            conn->send_buf[(conn->send_tail + i) % TCP_SEND_BUF_SIZE] = ptr[sent + i];
        }
        conn->send_tail = (uint16_t)((conn->send_tail + chunk) % TCP_SEND_BUF_SIZE);
        conn->send_count += chunk;
    ...
    for (uint32_t i = 0; i < TCP_MAX_CONNECTIONS; ++i) {
        ...
        if (c->state == TCP_STATE_ESTABLISHED && c->send_count > 0 &&
            ...
            if ((now - c->last_send_tick) >= retransmit_ticks) {
                ...
                for (uint32_t b = 0; b < unacked; ++b) {
                    retx_buf[b] = c->send_buf[(c->send_head + b) % TCP_SEND_BUF_SIZE];
                }
