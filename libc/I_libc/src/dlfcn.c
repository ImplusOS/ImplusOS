...
static int dl_path_exists(const char* path)
{
    int32_t fd = file_open(path, O_RDONLY);
    if (fd < 0) return 0;
    file_close(fd);
    ...
    char* out, size_t out_size)
{
    ...
    if (dependee != NULL && strchr(dependee, '/') != NULL) {
        ...
        if (dir_len + 1 + strlen(needed) + 1 < out_size) {
            ...
            if (dl_path_exists(out)) return 1;
        }
    }
    ...
    /* /Userland/Service/<name>/<name>.so -- each Userland service lives in
     * its own directory named after the soname stem (mirrors the search
     * order in the ld.so interpreter, Userland/Service/com.ImplusOS.ldso). */
    {
        size_t nlen = strlen(needed);
        size_t stem = nlen;
        if (stem > 3 && strcmp(needed + stem - 3, ".so") == 0) stem -= 3;
        if (17 + stem + 1 + nlen + 1 < out_size) {
            int n = snprintf(out, out_size, "/Userland/Service/%.*s/%s",
                             (int)stem, needed, needed);
            if (n > 0 && (size_t)n < out_size && dl_path_exists(out)) return 1;
        }
    }

    if (snprintf(out, out_size, "/lib/%s", needed) > 0 && dl_path_exists(out)) {
        return 1;
    }
