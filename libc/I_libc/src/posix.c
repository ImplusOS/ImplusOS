...
/* ---- ifaddrs ----
 * ImplusOS has a single IPv4 NIC plus loopback. There is no kernel API to
 * enumerate interface addresses, so the primary interface's address is
 * discovered with the standard connected-UDP-socket / getsockname() trick
 * (no packets are sent for a datagram socket), and loopback is synthesised.
 * Enough for interface enumeration / network-change detection to see a
 * usable IPv4 interface. */
...
struct ifaddrs_node {
    struct ifaddrs      pub;    /* must be first: freeifaddrs frees this ptr */
    char                name[16];
    struct sockaddr_in  addr;
    struct sockaddr_in  netmask;
};

static struct ifaddrs_node *ifaddrs_make(const char *name, uint32_t ip_be,
                                         uint32_t mask_be, unsigned int flags)
{
    struct ifaddrs_node *n = calloc(1, sizeof(*n));
    if (!n) {
        return NULL;
    }
    strncpy(n->name, name, sizeof(n->name) - 1);
    n->addr.sin_family = AF_INET;
    n->addr.sin_addr.s_addr = ip_be;
    n->netmask.sin_family = AF_INET;
    n->netmask.sin_addr.s_addr = mask_be;
...
int getifaddrs(struct ifaddrs **ifap)
{
    ...
    *ifap = NULL;
    ...
    struct ifaddrs_node *head = NULL;
    struct ifaddrs_node *tail = NULL;

    /* loopback */
    struct ifaddrs_node *lo = ifaddrs_make("lo", htonl(0x7f000001u),
                                           htonl(0xff000000u),
    ...
    /* primary IPv4 interface, discovered via getsockname() on a UDP socket
     * "connected" to an arbitrary public address. */
    ...
    if (s >= 0) {
        ...
        if (connect(s, (struct sockaddr *)&probe, sizeof(probe)) == 0 &&
            getsockname(s, (struct sockaddr *)&local, &ll) == 0 &&
            local.sin_addr.s_addr != 0u &&
            local.sin_addr.s_addr != htonl(0x7f000001u)) {
            struct ifaddrs_node *eth =
                ifaddrs_make("eth0", local.sin_addr.s_addr,
                             htonl(0xffffff00u), /* assume /24 */
                             IFF_UP | IFF_RUNNING | IFF_BROADCAST |
                                 IFF_MULTICAST);
            if (eth) {
                if (tail) {
                    tail->pub.ifa_next = &eth->pub;
                } else {
                    head = eth;
                }
                tail = eth;
...
void freeifaddrs(struct ifaddrs *ifa)
{
    while (ifa) {
        ...
        free(ifa); /* pub is the first member of ifaddrs_node */
        ifa = next;
    }
}
