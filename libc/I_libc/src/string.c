...
void* memset(void* ptr, int v, size_t n)
{
    ...
    size_t head = (-(uintptr_t)p) & 15u;
    if (head > n) head = n;
    for (size_t i = 0; i < head; i++)
        p[i] = c;

    uint64_t fill64 = (uint64_t)c * 0x0101010101010101ULL;
    uint64_t *wp = (uint64_t *)((uintptr_t)(p + head) & ~(uintptr_t)7);
    size_t skip = (size_t)((uintptr_t)wp - (uintptr_t)p);
    size_t remaining = n - skip;
...
int ffs(int i)
{
    if (i == 0)
        return 0;
    return (int)__builtin_ctz((unsigned int)i) + 1;
}
...
size_t strlen(const char* s)
{
    ...
    while (((uintptr_t)p & 7) != 0) {
        if (*p == '\0') return (size_t)(p - (const unsigned char*)s);
        p++;
    }
...
size_t strnlen(const char* s, size_t max_len)
{
    ...
    while (n < max_len && ((uintptr_t)p & 7) != 0) {
        if (*p == '\0') return n;
        p++;
        n++;
    }
