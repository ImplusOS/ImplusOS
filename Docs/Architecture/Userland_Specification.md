...
Applications follow reverse-domain naming: `com_ImplusOS_<appname>`
...
1. Application ELF files are placed on the boot filesystem under `Userland/SystemApps/` or `Userland/UserApps/`
...
### Window Manager (`com_ImplusOS_windowmanager`)
...
### DNS Resolver (`Userland/NetworkStack/DNS/DNS.c`)

Userland DNS resolver that performs DNS queries over UDP.

### Driver Framework API (`Userland/DriverFramework/API/`)
