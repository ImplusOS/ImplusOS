...
│   ├── Application/      System apps (com.ImplusOS.windowmanager, com.ImplusOS.sysnotif, ...)
│   ├── Service/          Hot-loadable Userland services, one ELF/.so each:
...
- `Build/Userland/<name>/<name>.ELF` — System applications
- `Build/Userland/<name>/<name>.ELF` — User applications
...
- **Userland services**: `Userland/Service/<name>/` holds hot-loadable services (ldso, dynmain, `com.ImplusOS.posix`, `com.ImplusOS.netstack`), each built to one ELF/`.so` and loadable/unloadable at runtime via `Userland/Service/service_client.h`.
- **POSIX layer**: `Userland/Service/com.ImplusOS.posix/` maps ImplusOS syscalls to POSIX API (open, read, write, fork, socket, pthread, etc.).
- **Application convention**: Each app lives in `Userland/Application/com.ImplusOS.<name>/` with its own `Makefile`.
...
| `Userland/Service/com.ImplusOS.posix/README_POSIX.md` | POSIX compatibility layer documentation |
