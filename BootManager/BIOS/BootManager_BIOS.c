...
static int load_kernel_elf(void *image, uint32_t image_size, uint32_t *entry_out, uint64_t slide, uint64_t *actual_base_out) {
    ...
    uint64_t base = min + slide;
    uint64_t load_bias = slide;

    for (uint16_t i = 0; i < eh->e_phnum; ++i) {
        if (ph[i].p_type != PT_LOAD) continue;
        if (ph[i].p_offset + ph[i].p_filesz > image_size || ph[i].p_memsz < ph[i].p_filesz) return -1;
        uint64_t dest = ph[i].p_paddr + load_bias;
        memcpy((void *)(uintptr_t)dest, (uint8_t *)image + ph[i].p_offset, (size_t)ph[i].p_filesz);
        memset((void *)(uintptr_t)(dest + ph[i].p_filesz), 0, (size_t)(ph[i].p_memsz - ph[i].p_filesz));
    }
        ...
        for (uint16_t i = 0; i < eh->e_shnum; ++i) {
            ...
            while (rela_offset + sizeof(Elf64_Rela) <= sh[i].sh_size) {
                ...
                if (type == R_X86_64_RELATIVE) {
                    uint64_t target_addr = cur->r_offset + load_bias;
                    uint64_t *target = (uint64_t *)(uintptr_t)target_addr;
                    *target = load_bias + (uint64_t)cur->r_addend;
                }
    ...
    *entry_out = (uint32_t)(eh->e_entry + load_bias);
    if (actual_base_out) *actual_base_out = base;
    return 0;
}
...
void bootmanager_bios_main(BIOS_BOOT_PARAMS *params) {
    ...
    uint32_t entry = 0;
    uint64_t kaslr_slide = kaslr_calculate_slide_range(0x10000000, 768, 0x200000);
    uint64_t actual_base = 0x01800000;
    if (load_kernel_elf((void *)(uintptr_t)BIOS_KERNEL_ELF_BUFFER, kernel_size, &entry, kaslr_slide, &actual_base) != 0) {
        for (;;) hal_cpu_halt();
    }

    memset(&g_boot_info, 0, sizeof(g_boot_info));
    g_boot_info.KaslrEnabled   = 1;
    g_boot_info.KernelVirtBase = actual_base;
    g_boot_info.KernelPhysBase = actual_base;
    g_boot_info.KernelSlide    = (actual_base >= 0x01800000) ? (actual_base - 0x01800000) : 0;
