...
static int arch_relative_reloc(boot_elf_arch_t arch, uint32_t type)
{
    if (arch == BOOT_ELF_ARCH_X86_64) return type == R_X86_64_RELATIVE;
    if (arch == BOOT_ELF_ARCH_AARCH64) return type == R_AARCH64_RELATIVE;
    return 0;
}

int boot_elf_load64(const boot_elf_load_request_t *req, boot_elf_load_result_t *out)
{
    if (!req || !out || !req->image || !req->alloc_pages_at) return -1;
    if (req->image_size < sizeof(Elf64_Ehdr)) return -1;
    ...
    if (eh->e_ident[4] != 2 || !arch_accepts_machine(req->arch, eh->e_machine)) {
        return -1;
    }
    if (eh->e_phentsize != sizeof(Elf64_Phdr) ||
        eh->e_phoff > req->image_size ||
    ...
    if (load_count == 0 || max <= min) return -1;
    ...
    for (uint16_t i = 0; i < eh->e_phnum; ++i) {
        ...
        memcpy((void *)(uintptr_t)dest, (uint8_t *)req->image + ph[i].p_offset, (size_t)ph[i].p_filesz);
    }
    ...
    if (eh->e_shoff != 0 && eh->e_shnum != 0) {
        ...
        for (uint16_t i = 0; i < eh->e_shnum; ++i) {
            ...
            if (sh[i].sh_offset > req->image_size || sh[i].sh_offset + sh[i].sh_size > req->image_size) {
                if (req->free_pages) req->free_pages(base, pages, req->ctx);
                return -1;
            }
            ...
            for (uint64_t j = 0; j < count; ++j) {
                if (!arch_relative_reloc(req->arch, rela_type(rela[j].r_info))) continue;
