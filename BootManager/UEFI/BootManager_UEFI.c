...
EFI_STATUS LoadKernelELF(
    ...
    *EntryPoint = Result.entry;
    ...
    if (BootInfo) {
        BootInfo->KaslrEnabled   = 1;
        BootInfo->KernelVirtBase = Result.base;
        BootInfo->KernelPhysBase = Result.base;
        BootInfo->KernelSlide    = (Result.base >= 0x01800000) ? (Result.base - 0x01800000) : 0;
    }
...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *ST) {
    ...
    EFI_STATUS LoadStatus = LoadKernelELF(ST, KernelBuffer, KernelSize, &KernelEntry, &BootInfo);
