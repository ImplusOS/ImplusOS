#include "AES_CBC_CTR.h"
#include <string.h>

static const uint8_t sbox[256] = {
    
};

static const uint8_t rsbox[256] = {
    
};

static const uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

static uint8_t xtime(uint8_t a)
{
    return (uint8_t)((a << 1) ^ (((a >> 7) & 1) * 0x1b));
}

static void aes128_key_expand(const uint8_t key[16], uint8_t rk[176])
{
    memcpy(rk, key, 16);
    for (int i = 4; i < 44; ++i) {
        uint8_t temp[4];
        temp[0] = rk[(i - 1) * 4 + 0];
        temp[1] = rk[(i - 1) * 4 + 1];
        temp[2] = rk[(i - 1) * 4 + 2];
        temp[3] = rk[(i - 1) * 4 + 3];
        if (i % 4 == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ rcon[i / 4];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
        }
        rk[i * 4 + 0] = rk[(i - 4) * 4 + 0] ^ temp[0];
        rk[i * 4 + 1] = rk[(i - 4) * 4 + 1] ^ temp[1];
        rk[i * 4 + 2] = rk[(i - 4) * 4 + 2] ^ temp[2];
        rk[i * 4 + 3] = rk[(i - 4) * 4 + 3] ^ temp[3];
    }
}

static void aes256_key_expand(const uint8_t key[32], uint8_t rk[240])
{
    memcpy(rk, key, 32);
    for (int i = 8; i < 60; ++i) {
        uint8_t temp[4];
        temp[0] = rk[(i - 1) * 4 + 0];
        temp[1] = rk[(i - 1) * 4 + 1];
        temp[2] = rk[(i - 1) * 4 + 2];
        temp[3] = rk[(i - 1) * 4 + 3];
        if (i % 8 == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ rcon[i / 8];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
        } else if (i % 8 == 4) {
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];
        }
        rk[i * 4 + 0] = rk[(i - 8) * 4 + 0] ^ temp[0];
        rk[i * 4 + 1] = rk[(i - 8) * 4 + 1] ^ temp[1];
        rk[i * 4 + 2] = rk[(i - 8) * 4 + 2] ^ temp[2];
        rk[i * 4 + 3] = rk[(i - 8) * 4 + 3] ^ temp[3];
    }
}

static void add_round_key(uint8_t state[16], const uint8_t rk[16])
{
    for (int i = 0; i < 16; ++i) state[i] ^= rk[i];
}

static void sub_bytes(uint8_t state[16])
{
    for (int i = 0; i < 16; ++i) state[i] = sbox[state[i]];
}

static void inv_sub_bytes(uint8_t state[16])
{
    for (int i = 0; i < 16; ++i) state[i] = rsbox[state[i]];
}

static void shift_rows(uint8_t state[16])
{
    uint8_t t;
    t = state[1]; state[1] = state[5]; state[5] = state[9];
    state[9] = state[13]; state[13] = t;
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    t = state[15]; state[15] = state[11]; state[11] = state[7];
    state[7] = state[3]; state[3] = t;
}

static void inv_shift_rows(uint8_t state[16])
{
    uint8_t t;
    t = state[13]; state[13] = state[9]; state[9] = state[5];
    state[5] = state[1]; state[1] = t;
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    t = state[3]; state[3] = state[7]; state[7] = state[11];
    state[11] = state[15]; state[15] = t;
}

static void mix_columns(uint8_t state[16])
{
    for (int i = 0; i < 4; ++i) {
        uint8_t *col = state + i * 4;
        uint8_t a = col[0], b = col[1], c = col[2], d = col[3];
        uint8_t a2 = xtime(a), b2 = xtime(b), c2 = xtime(c), d2 = xtime(d);
        col[0] = a2 ^ b ^ b2 ^ c ^ d;
        col[1] = a ^ b2 ^ c ^ c2 ^ d;
        col[2] = a ^ b ^ c2 ^ d ^ d2;
        col[3] = a2 ^ a ^ b ^ c ^ d2;
    }
}

static void inv_mix_columns(uint8_t state[16])
{
    for (int i = 0; i < 4; ++i) {
        uint8_t *col = state + i * 4;
        uint8_t a = col[0], b = col[1], c = col[2], d = col[3];
        uint8_t a2 = xtime(a), a4 = xtime(a2), a8 = xtime(a4);
        uint8_t b2 = xtime(b), b4 = xtime(b2), b8 = xtime(b4);
        uint8_t c2 = xtime(c), c4 = xtime(c2), c8 = xtime(c4);
        uint8_t d2 = xtime(d), d4 = xtime(d2), d8 = xtime(d4);
        col[0] = a8 ^ a4 ^ a2 ^ b8 ^ b2 ^ b ^ c8 ^ c4 ^ c ^ d8 ^ d4 ^ d2;
        col[1] = a8 ^ a4 ^ a ^ b8 ^ b4 ^ b2 ^ c8 ^ c2 ^ c ^ d8 ^ d4 ^ d;
        col[2] = a8 ^ a2 ^ a ^ b8 ^ b4 ^ b ^ c8 ^ c4 ^ c2 ^ d8 ^ d2 ^ d;
        col[3] = a8 ^ a4 ^ a ^ b8 ^ b2 ^ b ^ c8 ^ c4 ^ c ^ d8 ^ d4 ^ d2;
    }
}

static void aes_encrypt(const uint8_t rk[], const uint8_t in[16], uint8_t out[16], int nr)
{
    uint8_t state[16];
    memcpy(state, in, 16);
    add_round_key(state, rk);
    for (int r = 1; r < nr; ++r) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, rk + r * 16);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, rk + nr * 16);
    memcpy(out, state, 16);
}

static void aes_decrypt(const uint8_t rk[], const uint8_t in[16], uint8_t out[16], int nr)
{
    uint8_t state[16];
    memcpy(state, in, 16);
    add_round_key(state, rk + nr * 16);
    for (int r = nr - 1; r >= 1; --r) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, rk + r * 16);
        inv_mix_columns(state);
    }
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, rk);
    memcpy(out, state, 16);
}

static int aes_cbc_encrypt(const uint8_t *key, size_t key_len, const uint8_t iv[16],
                           const uint8_t *plaintext, size_t plaintext_len, uint8_t *ciphertext)
{
    if (key == NULL || iv == NULL || plaintext == NULL || ciphertext == NULL) return -1;
    if (plaintext_len == 0 || plaintext_len % 16 != 0) return -1;

    int nr = (key_len == 16) ? 10 : 14;
    uint8_t rk[240];

    if (key_len == 16) aes128_key_expand(key, rk);
    else aes256_key_expand(key, rk);

    uint8_t block[16];
    memcpy(block, iv, 16);
    uint8_t buffer[16];

    for (size_t i = 0; i < plaintext_len; i += 16) {
        for (int j = 0; j < 16; ++j) buffer[j] = plaintext[i + j] ^ block[j];
        aes_encrypt(rk, buffer, block, nr);
        memcpy(ciphertext + i, block, 16);
    }

    memset(rk, 0, sizeof(rk));
    return 0;
}

static int aes_cbc_decrypt(const uint8_t *key, size_t key_len, const uint8_t iv[16],
                           const uint8_t *ciphertext, size_t ciphertext_len, uint8_t *plaintext)
{
    if (key == NULL || iv == NULL || ciphertext == NULL || plaintext == NULL) return -1;
    if (ciphertext_len == 0 || ciphertext_len % 16 != 0) return -1;

    int nr = (key_len == 16) ? 10 : 14;
    uint8_t rk[240];

    if (key_len == 16) aes128_key_expand(key, rk);
    else aes256_key_expand(key, rk);

    uint8_t prev[16];
    memcpy(prev, iv, 16);
    uint8_t dec[16];

    for (size_t i = 0; i < ciphertext_len; i += 16) {
        aes_decrypt(rk, ciphertext + i, dec, nr);
        for (int j = 0; j < 16; ++j) plaintext[i + j] = dec[j] ^ prev[j];
        memcpy(prev, ciphertext + i, 16);
    }

    memset(rk, 0, sizeof(rk));
    return 0;
}

static int aes_ctr_encrypt(const uint8_t *key, size_t key_len, const uint8_t nonce[16],
                           const uint8_t *data, size_t data_len, uint8_t *out)
{
    if (key == NULL || nonce == NULL || data == NULL || out == NULL) return -1;

    int nr = (key_len == 16) ? 10 : 14;
    uint8_t rk[240];

    if (key_len == 16) aes128_key_expand(key, rk);
    else aes256_key_expand(key, rk);

    uint8_t counter[16];
    memcpy(counter, nonce, 16);
    uint8_t encrypted[16];
    size_t offset = 0;

    while (offset < data_len) {
        aes_encrypt(rk, counter, encrypted, nr);
        size_t todo = data_len - offset;
        if (todo > 16) todo = 16;
        for (size_t i = 0; i < todo; ++i) out[offset + i] = data[offset + i] ^ encrypted[i];
        offset += todo;
        for (int i = 15; i >= 0; --i) {
            if (++counter[i] != 0) break;
        }
    }

    memset(rk, 0, sizeof(rk));
    return 0;
}

int crypto_aes128_cbc_encrypt(const uint8_t key[16], const uint8_t iv[16],
                              const uint8_t *plaintext, size_t plaintext_len, uint8_t *ciphertext)
{ return aes_cbc_encrypt(key, 16, iv, plaintext, plaintext_len, ciphertext); }
int crypto_aes128_cbc_decrypt(const uint8_t key[16], const uint8_t iv[16],
                              const uint8_t *ciphertext, size_t ciphertext_len, uint8_t *plaintext)
{ return aes_cbc_decrypt(key, 16, iv, ciphertext, ciphertext_len, plaintext); }
int crypto_aes256_cbc_encrypt(const uint8_t key[32], const uint8_t iv[16],
                              const uint8_t *plaintext, size_t plaintext_len, uint8_t *ciphertext)
{ return aes_cbc_encrypt(key, 32, iv, plaintext, plaintext_len, ciphertext); }
int crypto_aes256_cbc_decrypt(const uint8_t key[32], const uint8_t iv[16],
                              const uint8_t *ciphertext, size_t ciphertext_len, uint8_t *plaintext)
{ return aes_cbc_decrypt(key, 32, iv, ciphertext, ciphertext_len, plaintext); }
int crypto_aes128_ctr_encrypt(const uint8_t key[16], const uint8_t nonce[16],
                              const uint8_t *data, size_t data_len, uint8_t *out)
{ return aes_ctr_encrypt(key, 16, nonce, data, data_len, out); }
int crypto_aes256_ctr_encrypt(const uint8_t key[32], const uint8_t nonce[16],
                              const uint8_t *data, size_t data_len, uint8_t *out)
{ return aes_ctr_encrypt(key, 32, nonce, data, data_len, out); }
