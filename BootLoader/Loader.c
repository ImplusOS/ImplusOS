...
static EFI_FILE_PROTOCOL *ResolveResourceRoot(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *ST) {
    ...
    if (Root) {
        ...
        Status = uefi_call_wrapper(Root->Open, 5, Root, &TestFile,
            L"EFI\\BOOT\\Resource\\Images\\BootLogo.bmp", EFI_FILE_MODE_READ, 0);
        if (!EFI_ERROR(Status)) {
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *ST) {
    ...
    DiscoverSMBIOS(ST, CPUName, Manufacturer, ProductName);

    char DisplayText[256] = {0};
    AppendString(DisplayText, "CPU: ");
    AppendString(DisplayText, CPUName[0]      ? CPUName      : "Unknown");
    AppendString(DisplayText, " | Maker: ");
    AppendString(DisplayText, Manufacturer[0] ? Manufacturer : "Unknown");
    AppendString(DisplayText, " | Model: ");
    AppendString(DisplayText, ProductName[0]  ? ProductName  : "Unknown");
    ...
    if (ResourceRoot) {
        ...
        Status = uefi_call_wrapper(ResourceRoot->Open, 5, ResourceRoot, &FontFile,
            L"EFI\\BOOT\\Resource\\Fonts\\NotoSansJP-Regular.ttf", EFI_FILE_MODE_READ, 0);
        if (!EFI_ERROR(Status)) {
